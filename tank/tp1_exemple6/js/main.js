...
function startGame() {
  ...
  let tank = scene.getMeshByName("tankBody");
...
function createTank(scene) {
  let tank = new BABYLON.MeshBuilder.CreateBox(
    "tankBody",
    { height: 3, depth: 5, width: 3 },
    scene
  );

  let tourelle = new BABYLON.MeshBuilder.CreateBox(
    "tourelle",
    { height: 1, depth: 3, width: 2 },
    scene
  );

  let canon = new BABYLON.MeshBuilder.CreateCylinder(
    "canon",
  ...
  tourelle.position.y = 3;
  tourelle.setParent(tank);

  canon.position.y = 3;
  canon.rotation.x = -5;
  canon.setParent(tourelle);
  //canon.setParent(tourelle);
